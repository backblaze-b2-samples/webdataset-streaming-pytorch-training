...
# B2_PUBLIC_URL_BASE is optional (public-bucket URLs only), so it is NOT
# listed here — a missing optional key must never block startup.
...
API_TITLE = "WebDataset Streaming PyTorch Training API"
API_DESCRIPTION = (
    "Local API for WebDataset Streaming PyTorch Training: packs media into "
    "WebDataset .tar shards, writes them and a JSON manifest to Backblaze B2, "
    "and streams them back into a PyTorch training loop with no local staging "
    "disk. This contract documents the template's local API, not a hosted "
    "public endpoint."
)
API_VERSION = "0.1.0"
